...
#CONSOLIDATE THIS INTO ONE TABLE ... FIX THIS CODE
...
names(StatsByPur)[names(StatsByPur)=="meanKm"] <- "Mean route in km"
...
clickMap1 <- leaflet(elodeaTRUE, width = "80%", height = 500) %>%
  ...
  addPolylines(data=Alltracks, stroke = TRUE, color = "#1b9e77", weight = 2,
               opacity = 1.0, fill = FALSE, group = "All boat tracks")%>%
  addPolylines(data=NotClean, stroke = TRUE, color = "#7570b3", weight = 2,
               opacity = 1.0, fill = FALSE, group = "No clean-drain-dry")%>%
  #Layers control
  addLayersControl(
